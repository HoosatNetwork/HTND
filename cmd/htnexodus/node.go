...
// `diff --live` only call read APIs against it. The node process must not be running against
// the same database directory at the same time, since the two would contend for the same
// underlying database files/locks.
