...
import (
	...
	"github.com/HoosatNetwork/HTND/util/network"
...
type Flags struct {
	...
	StopOnDisqualifiedStreak        bool          `long:"stop-on-disqualified-streak" hidden:"true" description:"Stop the node after 15 consecutive relayed or submitted blocks resolve to DisqualifiedFromChain. Off by default: the streak is always logged, but a run of disqualified blocks can be produced by anyone who mines blocks with a bad UTXO commitment on top of the tip, so stopping on it lets such a miner take nodes offline."`
